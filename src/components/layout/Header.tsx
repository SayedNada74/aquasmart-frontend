...
export function Header({ onMenuToggle }: HeaderProps) {
    ...
    const notifications = [
        ...
        { text: t("تم تشغيل البدالة تلقائياً", "Aerator activated auto"), type: "success", time: t("منذ ساعتين", "2 hours ago") },
    ];
        ...
        <header className="h-14 md:h-16 glass border-b border-[var(--color-border)] flex items-center justify-between px-4 md:px-6 flex-shrink-0">
            ...
            <div className="flex items-center gap-3 flex-1">
                ...
                <div className="relative hidden sm:block max-w-xs w-full">
                    ...
                    <input placeholder={t("بحث عن حوض أو مستشعر...", "Search ponds or sensors...")}
                        className="w-full bg-[var(--color-bg-input)] border border-[var(--color-border)] rounded-lg pl-9 pr-4 py-2 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-cyan)] focus:ring-1 focus:ring-[var(--color-cyan)]/30 transition-shadow" />
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-1 justify-end">
                ...
                <div className="relative" ref={notifRef}>
                    <button onClick={() => { setShowNotif(!showNotif); setShowProfile(false); }} className="relative w-9 h-9 rounded-lg bg-[var(--color-bg-input)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-cyan)] transition-colors">
                ...
                <div className="relative" ref={profileRef}>
                    <button onClick={() => { setShowProfile(!showProfile); setShowNotif(false); }} className="flex items-center gap-2">
