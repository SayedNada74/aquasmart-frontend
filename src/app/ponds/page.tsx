  ...
  useEffect(() => {
    ...
    const unsub = onValue(pondsRef, (snap) => {
      ...
      try {
        ...
        local.forEach((lp: PondData) => arr.push({ ...lp, isLocal: true }));
      } catch {
                ...
                <ChromaCard key={pond.id} className="card p-5 !border-transparent hover:!border-[var(--color-cyan)]/30 transition-all flex flex-col justify-between min-h-[300px]">
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between border-t border-[var(--color-border)] pt-3">
                      {pond.isLocal ? (
                        <button onClick={() => handleDeleteLocal(pond.id)} className="text-[#ef4444] text-xs hover:underline pointer-events-auto relative z-20">
                          {t("حذف", "Delete")}
                        </button>
                      ) : (
