...
export default function MarketPage() {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
                ...
                if (focusedPriceIndex === null) {
                    setFocusedPriceIndex((e.key === "ArrowRight" && !isRtl) || (e.key === "ArrowLeft" && isRtl) ? 0 : count - 1);
                    return;
                }
                ...
                if (e.key === "ArrowRight") {
                    nextIndex = isRtl 
                        ? (focusedPriceIndex > 0 ? focusedPriceIndex - 1 : count - 1)
                        : (focusedPriceIndex < count - 1 ? focusedPriceIndex + 1 : 0);
                } else {
                    nextIndex = isRtl 
                        ? (focusedPriceIndex < count - 1 ? focusedPriceIndex + 1 : 0)
                        : (focusedPriceIndex > 0 ? focusedPriceIndex - 1 : count - 1);
                }
                setFocusedPriceIndex(nextIndex);
    ...
    }, [data?.prices, focusedPriceIndex, lang]);
                        ...
                        {data.prices.map((f, i) => (
                            <motion.div 
                                ...
                                className={`card group transition-all duration-300 ${
                                    i === focusedPriceIndex 
                                    ? "border-[var(--color-cyan)] shadow-[0_0_20px_rgba(34,211,238,0.2)] scale-[1.02]" 
                                    : "hover:border-[var(--color-cyan)]/40 hover:scale-[1.02]"
                                }`}
                            >
                                <div className={`h-28 bg-[var(--color-bg-input)] rounded-lg mb-3 flex items-center justify-center text-5xl transition-transform ${i === focusedPriceIndex ? 'scale-110' : 'group-hover:scale-110'}`}>{f.image}</div>
                        ...
                        <div className="h-64">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={data.profitHistory}>
                                    ...
                                    <Bar dataKey="revenue" fill="var(--color-cyan)" radius={[4, 4, 0, 0]} name={t("الإيرادات", "Revenue")} barSize={20} />
