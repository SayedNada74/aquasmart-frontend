...
import { Waves, Shield, BarChart3, Sparkles, ChevronLeft, CheckCircle2, Play, Cpu, Bell, TrendingUp, Store, FlaskConical, Thermometer, Droplets, Wind, Menu, X } from "lucide-react";
    ...
    return (
        <PageTransition>
            <div className="min-h-screen text-[var(--color-text-primary)] bg-[var(--color-bg-base)]" dir={lang === "ar" ? "rtl" : "ltr"}>
                {/* ===== NAV ===== */}
                <nav className="flex items-center justify-between px-6 md:px-12 py-5 max-w-7xl mx-auto relative z-[100]">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={handleDashboardClick} className="btn-primary px-5 py-2 text-sm hidden sm:block">{t("لوحة التحكم", "Dashboard")}</button>
                    ...
                    {/* Mobile Menu Overlay */}
                    {isMenuOpen && (
                        <div className="fixed inset-0 top-[90px] bg-[var(--color-bg-base)]/95 backdrop-blur-2xl z-[200] flex flex-col p-8 md:hidden animate-in fade-in slide-in-from-top-4 duration-300 overflow-y-auto">
                            <div className="flex flex-col space-y-2">
                                {[
                                    { href: "#features", label: t("المميزات", "Features") },
                                    { href: "#ai", label: t("الذكاء الاصطناعي", "AI") },
                                    { href: "#market", label: t("السوق", "Market") },
                                ].map((item) => (
                                    <a 
                                        key={item.href}
                                        href={item.href} 
                                        className="text-2xl font-black py-4 border-b border-[var(--color-border)]/50 text-[var(--color-text-primary)] hover:text-[var(--color-cyan)] transition-colors"
                                ...
                                <button 
                                    ...
                                    className="text-2xl font-black py-4 border-b border-[var(--color-border)]/50 text-start text-[var(--color-text-primary)] hover:text-[var(--color-cyan)] transition-colors"
                                >
                                    {t("تواصل", "Contact")}
                                </button>
                            </div>
                            ...
                            <div className="mt-8 space-y-4">
                                <button 
                                    ...
                                    className="btn-primary w-full py-5 text-xl font-black shadow-[0_0_20px_rgba(34,211,238,0.3)]"
                                >
                                    {t("لوحة التحكم", "Dashboard")}
                                </button>
                                
                                <p className="text-center text-xs text-[var(--color-text-muted)] font-bold uppercase tracking-widest pt-4">
                                    AquaSmart AI • v1.0
                                </p>
                            </div>
                        </div>
