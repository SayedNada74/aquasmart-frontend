...
import { formatEGP } from "@/lib/valuation";
      ...
      <div className="space-y-6 pb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            ...
            { label: t("قيمة المزرعة", "Farm Value"), value: formatEGP(metrics.totalValue, lang), icon: <DollarSign className="w-5 h-5 text-[var(--color-cyan)]" />, iconBg: "bg-[var(--color-cyan)]/10" },
            ...
            <MotionCard key={index} className="stat-card">
              <div className="flex-1">
                <p className="text-[10px] text-[var(--color-text-secondary)] font-bold uppercase tracking-wider">{stat.label}</p>
                <p className="text-xl font-bold text-[var(--color-text-primary)] mt-1">{stat.value}</p>
              </div>
              <div className={`w-10 h-10 rounded-lg ${stat.iconBg} flex items-center justify-center flex-shrink-0`}>{stat.icon}</div>
              </MotionCard>
          ))}
        </div>
