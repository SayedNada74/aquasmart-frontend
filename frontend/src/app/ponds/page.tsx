...
import { PondDetailModal } from "@/components/ponds/PondDetailModal";
...
export default function PondsPage() {
  ...
  useEffect(() => {
    if (!showModal) {
      setActiveStep(0);
      setNewPond({ name: "", fishType: "Tilapia", area: "", feedTime: "07:00" });
    }
  }, [showModal]);
  ...
  useEffect(() => {
    ...
    const unsub = onValue(pondsRef, (snap) => {
      ...
      const arr: PondData[] = [];
      if (data) {
        Object.keys(data).slice(0, pondCount).forEach((key, idx) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
        const pondsCount = ponds.length;
        if (pondsCount === 0) return;

        let nextIndex = 0;
        if (selectedPond) {
          const currentIndex = ponds.findIndex(p => p.id === selectedPond.id);
          if (e.key === "ArrowRight") {
            nextIndex = isRtl 
                ? (currentIndex > 0 ? currentIndex - 1 : pondsCount - 1)
                : (currentIndex < pondsCount - 1 ? currentIndex + 1 : 0);
          } else {
            nextIndex = isRtl 
                ? (currentIndex < pondsCount - 1 ? currentIndex + 1 : 0)
                : (currentIndex > 0 ? currentIndex - 1 : pondsCount - 1);
          }
          setSelectedPond(ponds[nextIndex]);
        } else {
          // If no pond selected, start with first or last
          nextIndex = (e.key === "ArrowRight" && !isRtl) || (e.key === "ArrowLeft" && isRtl) ? 0 : pondsCount - 1;
          setSelectedPond(ponds[nextIndex]);
        }
      }
    };
  ...
  const handleDeleteLocal = (id: string) => {
    ...
    try {
      const existing = JSON.parse(localStorage.getItem("aquasmart_local_ponds") || "[]");
      localStorage.setItem("aquasmart_local_ponds", JSON.stringify(existing.filter((p: PondData) => p.id !== id)));
    } catch {
      // Ignore local storage write issues.
    }
        ...
        {ponds.length === 0 ? (
          <div className="card text-center py-16">
            ...
            <button onClick={() => setShowModal(true)} className="btn-primary text-sm">
              {t("+ إضافة حوض", "+ Add Pond")}
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            {ponds.map((pond) => {
              const pondIssue = getPondIssueDetails(pond.current, {
                Status: pond.status,
                Reason: pond.reason,
              });
              ...
              return (
                <ChromaCard 
                  key={pond.id} 
                  className="card p-5 !border-transparent hover:!border-[var(--color-cyan)]/30 transition-all flex flex-col justify-between min-h-[300px] cursor-pointer group"
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <h3 className="text-base font-bold text-[var(--color-text-primary)]">{t(pond.name_ar, pond.name_en)}</h3>
                      <Waves className="w-5 h-5 text-[var(--color-cyan)]" />
                    </div>
                  </div>

                  {pondIssue.severity !== "safe" && (
                    <div className={`mb-4 rounded-lg px-3 py-2 text-xs leading-relaxed ${pondIssue.severity === "danger" ? "bg-[#ef4444]/10 text-[#ef4444]" : "bg-[#f59e0b]/10 text-[#f59e0b]"}`}>
                      <span className="font-semibold">{t("سبب الحالة:", "Issue:")}</span>{" "}
                  ...
                  <div>
                    <div className="flex items-center justify-between text-[10px] text-[var(--color-text-muted)] mb-3">
                      <div className="flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {pond.feedSchedule}
                      </div>
                      <div className="flex items-center gap-1">
                        <Ruler className="w-3 h-3" /> {pond.area} m²
                      </div>
                    </div>

                    <div className="flex items-center justify-between border-t border-[var(--color-border)] pt-3">
                      {pond.isLocal ? (
                        <button onClick={(e) => { e.stopPropagation(); setPondToDelete(pond.id); }} className="text-[#ef4444] text-xs hover:underline pointer-events-auto relative z-20">
                          {t("حذف", "Delete")}
                        </button>
                      ) : (
                        <span className="flex items-center gap-1 text-[10px] text-[#10b981]">
                          <ShieldCheck className="w-3 h-3" />
                          {t("متصل بالمستشعرات", "Connected")}
                        </span>
                      )}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowModal(false)}>
              ...
              >
                        ...
                        <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="space-y-4">
                          ...
                          <div>
                            ...
                            <input
                              type="number"
                              value={newPond.area}
                              onChange={(e) => setNewPond((prev) => ({ ...prev, area: e.target.value }))}
                              placeholder="500"
                              min="1"
                              className="w-full bg-[var(--color-bg-input)] border border-[var(--color-border)] rounded-lg px-4 py-2.5 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-cyan)] focus:ring-1 focus:ring-[var(--color-cyan)] transition-shadow"
                            />
                        ...
                        <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="space-y-4">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => setNewPond((prev) => ({ ...prev, fishType: e.target.value }))}
                            ...
                            >
                              {fishTypes.map((fishType) => (
                                <option key={fishType.en} value={fishType.en}>
                                  {lang === "ar" ? `${fishType.ar} (${fishType.en})` : fishType.en}
                                </option>
                              ))}
                            </select>
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => setNewPond((prev) => ({ ...prev, feedTime: e.target.value }))}
                ...
                <div className="flex gap-3 p-5 border-t border-[var(--color-border)] rounded-b-2xl bg-[var(--color-bg-base)]/30 backdrop-blur-sm">
                  {activeStep > 0 ? (
                    <button onClick={() => setActiveStep((step) => step - 1)} className="btn-secondary flex-1 border border-[var(--color-border)] shadow-sm hover:bg-[var(--color-bg-input)]">
                      {t("السابق", "Back")}
                    </button>
                  ) : (
          ...
          {pondToDelete && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-3xl p-6 w-full max-w-sm shadow-2xl overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-8 opacity-5">
                  <Waves className="w-24 h-24" />
                </div>
                <h3 className="text-xl font-bold text-[var(--color-text-primary)] mb-2 relative z-10">
                  {t("تأكيد الحذف", "Confirm Deletion")}
                </h3>
                <p className="text-sm text-[var(--color-text-secondary)] mb-6 relative z-10">
                  {t("هل أنت متأكد من رغبتك في حذف هذا الحوض المحلي؟ لا يمكن التراجع عن هذا الإجراء.", "Are you sure you want to delete this local pond? This action cannot be undone.")}
                </p>
                <div className="flex gap-3 relative z-10">
                  <button
                    onClick={() => handleDeleteLocal(pondToDelete)}
                    className="flex-1 px-4 py-2.5 rounded-xl bg-[#ef4444] text-white font-bold text-sm hover:bg-[#dc2626] transition-colors"
                  >
                    {t("نعم، احذف", "Yes, Delete")}
                  </button>
                  <button
                    onClick={() => setPondToDelete(null)}
                    className="flex-1 px-4 py-2.5 rounded-xl bg-[var(--color-bg-input)] text-[var(--color-text-primary)] font-bold text-sm border border-[var(--color-border)] hover:bg-[var(--color-bg-card-hover)] transition-colors"
                  >
                    {t("إلغاء", "Cancel")}
                  </button>
                </div>
