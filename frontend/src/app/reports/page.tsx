...
export default function ReportsPage() {
    const { t, lang, userName, userEmail } = useApp();
    ...
    const handleExportFarmPDF = async () => {
        setExporting(true);
        try {
            const { generateFarmPDF } = await import("@/lib/generateFarmPDF");
            generateFarmPDF({
                period: period,
                managerName: userName,
                managerEmail: userEmail,
                ...
                activeAlerts: String(data.summaryStats.activeAlerts),
            ...
            console.error("Farm PDF export error:", err);
        } finally {
            setExporting(false);
        }
    };
            ...
            <div className="space-y-6 pb-8">
                {exportDone && (
                    <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-[#10b981] text-white px-6 py-3 rounded-xl shadow-xl flex items-center gap-2 animate-in slide-in-from-top-4 duration-300">
                        ...
                        <span className="font-medium">{t("تم تصدير التقرير بنجاح!", "Report exported successfully!")}</span>
                    </div>
                )}
                ...
                <div ref={registerSectionRef("export")} className={`flex items-center justify-between flex-wrap gap-4 ${getSectionHighlightClass("export")} rounded-xl`}>
                    ...
                    <div className="flex gap-3">
                        <button
                            onClick={handleExportFarmPDF}
                            disabled={exporting}
                            className="bg-[var(--color-bg-card)] text-[var(--color-text-primary)] border border-[var(--color-border)] flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl hover:bg-[var(--color-bg-input)] transition-all active:scale-95 disabled:opacity-50"
                        >
                            {exporting ? <Loader2 className="w-4 h-4 animate-spin" /> : <BarChart3 className="w-4 h-4 text-[var(--color-cyan)]" />}
                            {t("تقرير المزرعة الشامل", "Farm Executive Report")}
                        </button>
                        ...
                        >
                            ...
                            {t("تقارير الأحواض التفصيلية", "Detailed Pond Reports")}
                        </button>
                    </div>
                </div>
