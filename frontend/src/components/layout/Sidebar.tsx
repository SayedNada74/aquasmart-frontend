...
import {
    ...
    Sun, Moon, Languages, X, Utensils, Fish, LogOut
} from "lucide-react";
...
export function Sidebar({ mobileOpen, onClose }: SidebarProps) {
    ...
    const navItems: PillNavItem[] = [
        ...
        { id: "/feeding", label: t("التغذية الذكية", "Smart Feeding"), href: "/feeding", icon: <Utensils className="w-5 h-5" /> },
        { id: "/fish-guide", label: t("دليل الأسماك", "Fish Guide"), href: "/fish-guide", icon: <Fish className="w-5 h-5" /> },
        ...
        <>
            ...
            <Link href="/dashboard" onClick={() => onClose?.()} className="h-16 md:h-20 flex items-center justify-center gap-3 border-b border-[var(--color-border)] px-4 hover:bg-[var(--color-bg-card-hover)] transition-colors">
                <Image
                    ...
                    height={80}
                    priority
                    className="w-14 h-14 md:w-20 md:h-20 rounded-2xl shadow-sm object-contain"
                />
