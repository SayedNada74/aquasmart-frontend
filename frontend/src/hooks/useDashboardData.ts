...
import { fetchMarketData, FishPrice } from "@/lib/market/marketDataService";
import { calculatePondValuation } from "@/lib/valuation";
...
export function useDashboardData(location?: string) {
  const { pondCount } = useApp();
  const [ponds, setPonds] = useState<DashboardPondData[]>([]);
  const [alerts, setAlerts] = useState<DashboardAlert[]>([]);
  const [tasks, setTasks] = useState<ScheduledTask[]>([]);
  const [weather, setWeather] = useState<WeatherData | null>(null);
  const [marketPrices, setMarketPrices] = useState<FishPrice[]>([]);
  ...
  const metrics: DashboardMetrics = useMemo(() => {
    ...
    const totalValue = ponds.reduce((sum, pond) => {
        // Find if this pond has fishType metadata, otherwise default to tilapia
        const speciesId = pond.id === "pond_2" ? "mullet" : "tilapia"; // Simple mapping for now
        return sum + calculatePondValuation(10000, 250, speciesId, marketPrices);
    }, 0);
  ...
  }, [alerts, ponds, tasks, marketPrices]);
      ...
      try {
        const response = await fetch("/api/dashboard-summary", {
          ...
          headers: { 
                    ...
                    [API_AUTH_HEADER]: getApiClientKey()
                },
          body: JSON.stringify({
